...
# updated 15/01/21           - READ README.md for changelog.
#
...
if {[catch {source [file dirname [info script]]/tcleggdrop_mcps_sitesettings.ini} error]} {
	putloglev o * "\00304\[MCEggdrop - erreur\]\003 error: $error"
	return
}
#######################################################

if { $::mcplaintextblock == "true" } {
	if {[catch {source [file dirname [info script]]/PlaintextBlock.tcl} error]} {
		putloglev o * "\00304\[MCEggdrop - erreur\]\003 error: $error"
		return
	}
}
#######################################################
# Check versions
set min_eggdrop_vers	"1.8.2"
if { [package vcompare [regexp -inline -- {^[[:digit:]\.]+} $::version] $min_eggdrop_vers] == -1 } { 
	putloglev o * "\00304\[MCEggdrop - erreur\]\003 La version de votre Eggdrop est\00304 ${::version}\003; MCEggdrop ne fonctionnera correctement que sur les Eggdrops version $min_eggdrop_vers ou supérieure." ;
	return
}
#######################################################
...
proc stdrag_trybind { comtype nick uhost hand chan text table flags mask hits callback} {
	...
	if { $table == "pubm" } {
		if { ![regexp -nocase -- $cmask [list $chan $text]] } {
...
proc putnowmc {args} {
	...
	if {$colonpos == -1 || $colonpos == 0} {
		# some raw commands to irc dont have : and they should be sent unencrypted
		eval trueputnow [list $putlinetext] $putlineargs
		return
	}
	# grab the initial 'PRIVMSG CHAN :'
	set partone [ string range $putlinetext 0 [expr $colonpos - 1 ] ]
	...
	set msg [ string range $putlinetext [ expr $colonpos + 1 ] end ]
	# NEW REPLACEMENT
	encryptout trueputnow $prefixkeyword $dest $msg $putlineargs
	# encrypt it
	set newmsg [ mcpsencrypt $dest $msg ]
	#now send the rebuilt string
	if { $newmsg != "" } {
...
proc trueputnow {args} {
	global mcautomagic
	if {$mcautomagic != "true"} { eval putnow $args } else { eval _mcorig_putnow $args }
}
...
proc mcpsdecrypt {c t} {
	...
	if {$firstword == "mcps" || $firstword == "+OK"} {
		...
		set methodencrypt ""
		if {[info exists mcpsmethod(defaultencrypt)] != 0} { set methodencrypt $mcpsmethod(defaultencrypt) }
		if {[info exists mcpsmethod($c)] != 0} { set methodencrypt $mcpsmethod($c) }
		
		#lowercase methodencrypt
		set ::blowfish-use-mode [string tolower $methodencrypt]
		...
		if {$chankey != ""} {
			...
			if {[string match {cbc[:;]*} $chankey]} {
				...
				set t [decrypt $chankeyfixed $t]
			} elseif {[string match {ecb[:;]*} $chankey]} {
				# ecb decrpyt
				set chankeyfixed [string range $chankey 4 end]
				# strip off the leading *
				set t [string range $t 1 end]
